...
import com.example.candle.service.BucketCalculator;
...
public class CandleAggregationServiceImpl implements CandleAggregationService,SmartLifecycle{

	private static final Logger log = LoggerFactory.getLogger(CandleAggregationServiceImpl.class);
		...
		for (Interval interval : candleProperties.getSupportedIntervalEnums()) {
			...
			if (existing != null) {
				if (existing.getBucketStartEpochMillis() == bucketStart) {
					...
					log.debug("Updated live candle: symbol={}, interval={}, bucketStart={}, close={}",
                            symbol, interval, bucketStart, existing.getClose());
					continue; // done
				}
			}
			...
			if (existing != null) {
				...
				log.debug("Closing candle: symbol={}, interval={}, bucketStart={}, o/h/l/c={}/{}/{}/{} vol={}",
                        existing.getSymbol(), existing.getInterval(),
                        existing.getBucketStartEpochMillis(),
                        existing.getOpen(), existing.getHigh(), existing.getLow(), existing.getClose(),
                        existing.getVolume());
			}

			// create new live candle
			symbolMap.put(interval,
					new LiveCandle(symbol, interval, bucketStart, price));
		}
	}
